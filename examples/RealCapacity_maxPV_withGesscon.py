...
class MaximizePV():
    ...
    def __init__(self,timeresolution,horizon,solver,
        ...
        for s_ess,s_ev,s_pos in product(self.ess_soc_states,self.ev_soc_states,self.ev_pos_states):
            self.Value[self.T,s_ess,s_ev,s_pos]=1.0
            
        """
        Might-be irrelevant part
        """
        #Parameters for ESS optimization
        self.LocalTargetWeight=local_em_weight
        self.GlobalTargetWeight=global_em_weight
        
        self.P_Load_Forecast=forecast_load
        self.P_PV_Forecast=forecast_pv
        self.Price_Forecast=forecast_price
        self.DSO_Command=dso_command
    ...
    def optimaldecisioncalculator(self,timestep,ini_ess_soc,ini_ev_soc,ini_pos):
        ...
        if ini_pos==1:
            ...
            def home_demandmeeting(model):
                ...
                return float(self.P_Load_Forecast[timestep])+model.P_EV+model.P_ESS==model.P_PV+model.P_GRID
            ...
            def objrule1(model):
                ...
                for p_ess,p_ev in product(model.decision_ess,model.decision_ev):   #If EV is charged with one of the feasible decision 'p_ev'
                    ...
                    powerFromEss=-p_ess/100*self.ESS_Capacity/self.dT
                    ...
                    immediate_cost=self.GlobalTargetWeight*(float(self.DSO_Command[timestep])-powerFromEss)*(float(self.DSO_Command[timestep])-powerFromEss)
    
                    objective_value+=model.Decision[p_ess]*(immediate_cost+expected_future_cost)    #Adding the expected_future cost of taking 'p_ess and p_ev' decision when initial condition is combination of 'ini_ess_soc','ini_ev_soc' and home state                                      
            
                return self.LocalTargetWeight*(float(self.P_PV_Forecast[timestep])-model.P_PV)*(float(self.P_PV_Forecast[timestep])-model.P_PV)+objective_value 
...
df_LA  = xl.parse("0")
df_DSO = xl_dso.parse("0")
forecasts=df_LA.to_dict('series')
dso_commands=df_DSO.to_dict('series')
