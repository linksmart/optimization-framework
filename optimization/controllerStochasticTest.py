...
class OptControllerStochasticTest(ControllerBase):
    ...
    def optimize(self, action_handle_map, count, optsolver, solver_manager):
        while not self.redisDB.get_bool(self.stop_signal_key):
            ...
            self.logger.debug("PV generation:", p_pv)
            self.logger.debug("Import:", p_grid)
            self.logger.debug("ESS discharge:", p_ess)
            self.logger.debug("VAC charging", p_vac)
            ...
            self.logger.debug("Import:", p_grid)
            self.logger.debug("ESS discharge:", p_ess)
            self.logger.debug("Real EV charging", feasible_ev_charging_power)
