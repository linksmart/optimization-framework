...
class Model:
	...
	model.SoC_Value = Param(model.N, within=PositiveReals)
	...
	model.SoC_Copy = Var(model.N, model.T_SoC, within=NonNegativeReals, bounds=(model.ESS_Min_SoC, model.ESS_Max_SoC))
	...
	def con_rule_soc(model):
		return (model.SoC_copy[n] == model.SoC_Value[n] for n in model.N)
	
	model.con_soc_copy = Constraint(model.N,  rule=con_rule_soc)
