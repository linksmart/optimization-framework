...
class Model:
	...
	model.Penalty_Factor = Param(within=NonNegativeReals)  # Active power demand
	...
	model.P_ESS_Output = Var(model.T, within=Reals,
							 bounds=(-model.ESS_Max_Charge_Power, model.ESS_Max_Discharge_Power),initialize=0)  # ,initialize=iniSoC)
	...
	def con_rule_is_grid_export(model,t):
		return model.P_Grid_Output[t] <= 0
	...
	model.is_grid_export = Expression(model.T, rule=con_rule_is_grid_export)
