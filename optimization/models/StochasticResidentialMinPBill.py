...
class Model:
    ...
    model.Price_single = Var(within=NonNegativeReals)
    ...
    def rule_iniPrice(model):
        for j in model.Price:
            if j == model.Timestep:
                return model.Price_single == model.Price[j]
    ...
    def objrule1(model):
        ...
        if model.Recharge == 1:
            ...
            return model.Price_single * model.P_GRID_OUTPUT + future_cost
