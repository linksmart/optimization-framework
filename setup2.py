# -*- coding: utf-8 -*-
"""
Created on Tue Feb 20 15:22:17 2018
...
Downloads
    ...
    PyPA
    Python Packaging Authority Projects
    wheel
    Pull requests

+Python 3.6, -Python2.6, improved -b option, py.test tweaks
#82 Open
...
Description
        ...
        copy -b option to 'build' subcommand to support out-of-tree builds

        remove init.py from test directory
        ...
        Drops support for Python 2.6. Remove now unnecessary import workarounds.

        Add missing trove classifiers.

        rename [pytest] to [tool.pytest] in setup.cfg

Comments (2)

    Daniel Holth author

    Maybe you can take a look at the tests in 3.6. I removed init.py from the test directory to make py.test happier for most non-Py3.6 Pythons, but on Python 3.6 it's getting a weird error related to finding paths relative to a NamespacePackage using pkg_resources.resource_filename(). It might be easiest to change the way it looks for the test data.
        2017-07-08
    Alex Grönholm

    I don't quite understand what this is for. What are we missing without this patch?
        2017-07-29
...
Files changed (4)
    ...
    C wheel/__init__.py
    +473 -0
    C wheel/bdist_wheel.py Comments 1
    +25 -0
    C wheel/test/test_tool.py

File setup.cfg Conflict
View file

Conflict: Modified in source but deleted in destination
...
File wheel/__init__.py Conflict
View file
...
File wheel/bdist_wheel.py Conflict
View file
...
+            build_cmd.build_base = self.bdist_dir

+

+        bdist_cmd = self.get_finalized_command('bdist')

+        bdist_base = bdist_cmd.bdist_base

+        self.bdist_dir = os.path.join(bdist_base,

+                                      'bdist_wheel.' + bdist_cmd.plat_name)
...
+        # copy options from 'bdist' to 'bdist_wheel'
...
File wheel/test/test_tool.py Conflict
View file
