...
class Model:
	    ...
	    def combinatorics(model):
	        ...
	        return 1 == sum(model.Decision[ess, vac] for ess, vac in
	                        itertools.product(model.Feasible_ESS_Decisions, model.Feasible_VAC_Decisions))
	    ...
	    def ess_chargepower(model):
	        return model.P_ESS_OUTPUT == sum(model.Decision[ess, vac] * ess for ess, vac in
	                                         itertools.product(model.Feasible_ESS_Decisions,
	    ...
	    def objrule1(model):
	        ...
	        for ess, vac in itertools.product(model.Feasible_ESS_Decisions, model.Feasible_VAC_Decisions):
